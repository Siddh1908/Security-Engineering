...
Your task:
-----------
Implement the missing parts of DES. 
We have provided the S-Boxes, permutation tables, and padding functions.

Functions you must implement:
- string_to_bit_array
- bit_array_to_string
- xor
- generatekeys (finish key scheduling)
- compute_s_box (use S-Boxes)

Everything else is scaffolded for you.

Hints:
- DES works on 64-bit blocks (8 bytes).
- The key schedule generates 16 round keys (48 bits each).
- Use Latin-1 encoding for safe byte<->string mapping.
"""
...
#Permutation applied on shifted key to get Ki
CP_2 = [14, 17, 11, 24, 1, 5,
        3, 28, 15, 6, 21, 10,
        23, 19, 12, 4, 26, 8,
        16, 7, 27, 20, 13, 2,
        41, 52, 31, 37, 47, 55,
...
#Final permutation for datas after the 16 rounds
...
#Permutation used on the substituted block to permute the 32 bits
P = [16, 7, 20, 21,
     29, 12, 28, 17,
     1, 15, 23, 26,
     5, 18, 31, 10,
...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        # Apply the corresponding S-box (S1..S8) to each 6-bit chunk in order.
        result = []
    ...
    def permut(self, block, table):  # Permute the given block using the given table
    ...
    def generatekeys(self):
        ...
        # [FIX in TODO]
        self.keys = []
        key_bits = string_to_bit_array(self.password)
        key_bits = self.permut(key_bits, CP_1)  # 56 bits
        g, d = nsplit(key_bits, 28)
        for shift_amt in SHIFT:
            g, d = self.shift(g, d, shift_amt)
            combined = g + d
            subkey = self.permut(combined, CP_2)  # 48 bits
